...
1. **FLS is NOT auto-granted via Metadata API.** When deploying custom fields with `sf project deploy start`, Field-Level Security is NOT automatically granted to any profile — not even System Administrator. You MUST deploy Admin profile metadata alongside custom fields or the fields will exist but be invisible. See [field-level-security.md](references/tasks/field-level-security.md).
  ...
  Repo note (profiles): when you need to add `<fieldPermissions>` into an existing profile XML in this repo, do **not** assume the top-of-file structure is uniform across profiles. The most reliable insertion anchor is immediately after the existing `<custom>true</custom>` element (wherever it appears).

  Quick FLS checklist:
  - Ensure `force-app/main/default/profiles/Admin.profile-meta.xml` exists and contains a `<fieldPermissions>` entry for the new field.
  - For any additional profiles, add exactly one `<fieldPermissions>` block per field (read-only vs editable as required).
  - Dry-run deploy the field + the profile files together; if the field is “missing” later, it’s almost always FLS.
...
| Creating users, permission set groups, muting permission sets, or org-wide default sharing | [users-and-sharing.md](references/tasks/users-and-sharing.md) |
